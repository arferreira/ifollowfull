(function($){
 $.fn.horizontalScroll = function(options) {
  ...
  var elementwidth =  $(".element").width() + 35;
  var countelement =  $(".element").length;
  var containerwidth =  (elementwidth * countelement) - 35;

  $("#horiz_container").width(containerwidth);
...
// DW Event
// Fixes by Tobias Curth, TCU
// fp is sometimes undefined -> problems with older browser (or better with IE8)
var dw_Event = {

    add: function(obj, etype, fp, cap) {
        cap = cap || false;

        if (obj.addEventListener && fp) obj.addEventListener(etype, fp, cap);
        else if (obj.attachEvent && fp) obj.attachEvent("on" + etype, fp);
    },

    remove: function(obj, etype, fp, cap) {
        cap = cap || false;
        if (obj.removeEventListener && fp) obj.removeEventListener(etype, fp, cap);
        else if (obj.detachEvent && fp) obj.detachEvent("on" + etype, fp);
    },
